...
inline void subplots_adjust(double wspace, double hspace)
{
    PyObject *kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "wspace", PyFloat_FromDouble(wspace));
    PyDict_SetItemString(kwargs, "hspace", PyFloat_FromDouble(hspace));
    ...
    if(!res) throw std::runtime_error("Call to subplots_adjust() failed.");

    Py_DECREF(kwargs);
    Py_DECREF(res);
}

inline void axvline(double x, const std::map<std::string, std::string>& keywords)
{
